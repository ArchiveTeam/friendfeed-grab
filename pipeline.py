...
LUA_SHA1 = get_hash(os.path.join(CWD, 'friendfeed.lua'))
...
#            "--page-requisites",
        ...
        assert item_type in ('account', 'accountfull')
        
        if item_type == 'account':
            wget_args.append('http://friendfeed.com/{0}'.format(item_value))
        elif item_type == 'accountfull':
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="friendfeed"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
